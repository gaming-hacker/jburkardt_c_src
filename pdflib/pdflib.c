...
/*
  ...
  Parameters:
    ...
    Input, double P[M], the probability of each outcome on one trial.
    0.0 <= P(I) <= 1.0
    sum P = 1.0

    Input, int M, the number of possible outcomes on one trial.
    0 < M.

    Input, int X[M], the number of times each outcome occurred.
    0 <= X(I) <= N
    sum X = N
