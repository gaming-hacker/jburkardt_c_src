...
/*
  Purpose:
    ...
    MAIN is the main program for TEMPERATURE_SCATTER3D.

  Discussion:

    TEMPERATURE_SCATTER3D creates a 3D scatter plot of temperature.
...
*/
{
  ...
  char plot_filename[] = "temperature_scatter3d.png";

  timestamp ( );
  printf ( "\n" );
  printf ( "TEMPERATURE_SCATTER3D:\n" );
  printf ( "  C version\n" );
  printf ( "  Create a 3D scatter plot of\n" );
...
*/
  ...
  fprintf ( command, "set zlabel '<--- January Temperature --->'\n" );
  fprintf ( command, "set title 'January temperatures across US'\n" );
  fprintf ( command, "set grid\n" );
  fprintf ( command, "unset key\n" );
  fprintf ( command, "splot '%s' using 2:3:1 with points pt 7 ps 2\n", data_filename );
