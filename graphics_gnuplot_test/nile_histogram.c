...
/*
  Purpose:
    ...
    MAIN is the main program for NILE_HISTOGRAM.

  Discussion:

    NILE_HISTOGRAM creates a histogram of Nile flood heights.
...
*/
{
  ...
  char graphic_filename[] = "nile_histogram.png";

  timestamp ( );
  printf ( "\n" );
  printf ( "NILE_HISTOGRAM:\n" );
  printf ( "  C version\n" );
...
*/
  ...
  fprintf ( command, "set xlabel '<--- Height of Nile at Maximum Flood --->'\n" );
  ...
  fprintf ( command, "binwidth = 0.25\n" );
  fprintf ( command, "binstart = 5.0\n" );
  fprintf ( command, "set boxwidth 0.9*binwidth\n" );
  fprintf ( command, "plot '%s' i 0 using (binwidth*(floor(($2-binstart)/binwidth)+0.5)+binstart):(1.0) smooth freq with boxes lc rgb 'blue'\n", data_filename );
