...
/*
  Purpose:
    ...
    MAIN is the main program for PRESIDENT_HEIGHTS_HISTOGRAM.

  Discussion:

    PRESIDENT_HEIGHTS_HISTOGRAM creates a histogram of 90 values.
...
*/
{
  ...
  char graphic_filename[] = "president_heights_histogram.png";

  timestamp ( );
  printf ( "\n" );
  printf ( "PRESIDENT_HEIGHTS_HISTOGRAM:\n" );
  printf ( "  C version\n" );
...
*/
  ...
  fprintf ( command, "set xlabel '<--- Height (inches) --->'\n" );
  ...
  fprintf ( command, "set yrange [0:10]\n" );
  fprintf ( command, "unset key\n" );
  fprintf ( command, "binwidth = 1.0\n" );
  fprintf ( command, "binstart = 64.0\n" );
