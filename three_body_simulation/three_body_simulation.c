...
void three_body_simulation_run ( );
...
/*
  Purpose:
    ...
    MAIN is the main program for three_body_simulation.

  Discussion:

    three_body_simulation uses RKF45 as an integrator for the simple version
...
/*
  Purpose:
    ...
    three_body_simulation_RUN runs the simple three body ODE system.
...
*/
{
  ...
  int step_num = 630;
  double t;
  char t_filename[] = "three_body_simulation_t.txt";
  double t_out;
  double t_start;
  double t_stop;
  double *ts;
  double *y;
  char y_filename[] = "three_body_simulation_y.txt";
  double *yp;
  double *ys;
  ...
  printf ( "\n" );
  printf ( "three_body_simulation_RUN\n" );
  ...
  for ( step = 1; step <= step_num; step++ )
  {
    ...
    {
      printf ( "\n" );
      printf ( "three_body_simulation_RUN - Warning\n" );
