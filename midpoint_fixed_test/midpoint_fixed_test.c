...
# include "midpoint_fixed.h"

int main ( );
double *predator_prey_deriv ( double t, double rf[] );
void predator_prey_midpoint_fixed_test ( double tspan[2], double p0[2], int n );
double *stiff_deriv ( double t, double y[] );
double *stiff_exact ( int n, double t[] );
void stiff_midpoint_fixed_test ( double tspan[2], double y0[1], int n );
...
*/
{
  ...
  timestamp ( );
  printf ( "\n" );
  printf ( "midpoint_fixed_test:\n" );
  printf ( "  C version\n" );
  printf ( "  Test midpoint_fixed() on several ODE's.\n" );
...
/*
  Purpose:
    ...
    predator_prey_midpoint_fixed_test solves predator-prey using midpoint_fixed().
...
*/
{
  ...
  FILE *data;
  char header[] = "predator_prey_midpoint_fixed";
  int i;
  const int m = 2;
  double *pout;
  double *t;
  ...
  printf ( "\n" );
  printf ( "predator_prey_midpoint_fixed_test\n" );
...
*/
  ...
  printf ( "\n" );
  printf ( "  predator_prey_midpoint_fixed_test: data stored in \"%s\".\n", data_filename );
/*
...
*/
  ...
  fprintf ( command, "set title 'predator prey: midpoint fixed method'\n" );
  ...
  printf ( "  predator_prey_midpoint_fixed_test: plot commands stored in \"%s\".\n", 
    command_filename );
...
/*
  Purpose:
    ...
    stiff_midpoint_fixed() uses midpoint_fixed() on the stiff ODE.
...
*/
{
  ...
  printf ( "\n" );
  printf ( "stiff_midpoint_fixed_test\n" );
  printf ( "  Solve stiff ODE using midpoint_fixed().\n" );

  t1 = ( double * ) malloc ( ( n + 1 ) * sizeof ( double ) );
  y1 = ( double * ) malloc ( ( n + 1 ) * sizeof ( double ) );

  midpoint_fixed ( stiff_deriv, tspan, y0, n, m, t1, y1 );

  t2 = r8vec_linspace_new ( n2, tspan[0], tspan[1] );
  y2 = stiff_exact ( n2, t2 );

  plot2 ( n+1, t1, y1, n2, t2, y2, "stiff_midpoint_fixed", 
    "Stiff ODE: midpoint fixed method" );
