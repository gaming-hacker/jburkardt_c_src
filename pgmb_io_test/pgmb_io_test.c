...
/*
  Purpose:
    ...
    MAIN is the main program for PGMB_IO_TEST.
...
*/
{
  ...
  {
    printf ( "\n" );
    printf ( "PGMB_IO_TEST - Fatal error!\n" );
...
{
  bool error;
  char *file_out_name = "pgmb_io_test_01.pgm";
  unsigned char *g;
  int i;
...
*/
  ...
  if ( error )
  {
    printf ( "\n" );
    printf ( "TEST01 - Fatal error!\n" );
    printf ( "  PGMB_READ_TEST failed!\n" );
  }
  else
  {
    printf ( "\n" );
    printf (  "  PGMB_READ_TEST was successful.\n" );
  }
...
{
  bool error;
  char *file_in_name = "pgmb_io_test_02.pgm";
  unsigned char *g;
  unsigned char maxg;
  int xsize;
  int ysize;
